...
def test_state_store_marks_and_loads_cursors(tmp_path: Path) -> None:
    ...
    asyncio.run(loaded.mark_pending(10, 100))
    assert not asyncio.run(loaded.seen(10, 100))
    assert asyncio.run(loaded.pending(10)) == (100,)
    reloaded_pending = StateStore.load(path)
    assert asyncio.run(reloaded_pending.pending(10)) == (100,)
    asyncio.run(reloaded_pending.mark_processed(10, 100))
    assert asyncio.run(reloaded_pending.pending(10)) == ()
    assert asyncio.run(reloaded_pending.seen(10, 100))
    loaded = reloaded_pending
